...
class TestSearchScope < Test::Unit::TestCase
  context "Simple, mixed type tree" do
    setup do
      ...
      shape.children << circle = Circle.create(:name => "Circle")
      shape.children << square = Square.create(:name => "Square")
      ...
      @shape, @circle, @square = Shape.find(shape.id), Circle.find(circle.id), Square.find(square.id)
    end
  ...
  context "A tree with mixed types on either side of a branch" do
    setup do
      shape = Shape.create(:name => "Root")
      shape.children << circle = Circle.create(:name => "Circle")
      circle.children << square = Square.create(:name => "Square")
    ...
    should "destroy descendants of shape" do
      ...
      assert_nil Shape.where(:id => @circle._id).first
      assert_nil Shape.where(:id => @square._id).first
    end
  end
